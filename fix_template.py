                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="col-span-2 bg-gray-50 p-4 rounded-lg border">
                        ...
                        <div class="flex flex-wrap gap-4 mb-3">
                            ...
                            <label class="inline-flex items-center">
                                <input type="radio" name="water_billing_type" value="{{ code }}" class="form-radio text-blue-600" {% if form.water_billing_type.value == code %}checked{% endif %} required onchange="toggleDescription('water', this.value)">
                        ...
                        <div id="water_description_container"
                            class="mt-2 {% if form.water_billing_type.value != 'FIXO' %}opacity-50{% endif %}">
                            <label class="block text-sm text-gray-600 mb-1">Descrição Água (Habilitado apenas para Valor
                                Fixo)</label>
                            <input type="text" name="water_description" id="water_description"
                                value="{{ form.water_description.value|default:'' }}"
                                class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-200"
                                {% if form.water_billing_type.value != 'FIXO' %}disabled{% endif %}>
                        </div>
                    </div>
                    ...
                    <div class="col-span-2 bg-gray-50 p-4 rounded-lg border mt-4">
                        ...
                        <div class="flex flex-wrap gap-4 mb-3">
                            ...
                            {% if code %}
                            <label class="inline-flex items-center">
                                <input type="radio" name="power_billing_type" value="{{ code }}" class="form-radio text-blue-600" {% if form.power_billing_type.value == code %}checked{% endif %} required onchange="toggleDescription('power', this.value)">
                                <span class="ml-2">{{ label }}</span>
                            </label>
                            {% endif %}
                            {% endfor %}
                        ...
                        <div id="power_description_container"
                            class="mt-2 {% if form.power_billing_type.value != 'FIXO' %}opacity-50{% endif %}">
                            <label class="block text-sm text-gray-600 mb-1">Descrição Energia (Habilitado apenas para
                                Valor Fixo)</label>
                            <input type="text" name="power_description" id="power_description"
                                value="{{ form.power_description.value|default:'' }}"
                                class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-200"
                                {% if form.power_billing_type.value != 'FIXO' %}disabled{% endif %}>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    function toggleDescription(type, value) {
        const inputId = type + '_description';
        const containerId = type + '_description_container';
        ...
        if (value === 'FIXO') {
            ...
            input.disabled = true;
            input.value = ''; // Limpa o valor se não for fixo
            container.classList.add('opacity-50');
        }
    }

    // Inicializa o estado ao carregar a página (caso haja erro de validação e o form retorne)
    document.addEventListener('DOMContentLoaded', function () {
        const waterValue = document.querySelector('input[name="water_billing_type"]:checked')?.value;
        if (waterValue) toggleDescription('water', waterValue);

        const powerValue = document.querySelector('input[name="power_billing_type"]:checked')?.value;
        if (powerValue) toggleDescription('power', powerValue);
    });
</script>
{% endblock %}
"""
